	...
	for(i=1;i<=ngrav;i++){			
		...
		for(k=1;k<=NXG;k++){
			for(j=j_sea;j<=NYG;j++){
				...
				/* TAPER-TEST: PRECONDITIONING OFF --> TAPER IN TAPER_GRAD IS DEPTH SQUARED */
				/* Preconditioning of the kernel K by depth weighting function */
				K *= (double) pow((y+0.0005),(double) 0.8);
				
				
				/* calculate gradient with adjoint operator*/
				/*grad_grav_glob[j][k] += (double) (gz_res[i]/pow(r_center,(double) 2) * K  );*/
				
				/* calculate gradient with L2-norm*/
