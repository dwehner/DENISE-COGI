...
void grav_mod(float  **rho, int ngrav, float **gravpos, float *gz_mod, int NXGRAV, int NYGRAV, int NZGRAV){
	...
	/* background density for gravity modelling*/
	/*rho_mean=vector(1,NYG);
			...
			for(j=1;j<=NYG;j++){
				...
				for(l=1;l<=2;l++){
					for(m=1;m<=2;m++){
						...
						/*if(GRAV_TYPE==1){
						gztmp += (double)(s*G*(rho[j][k]-rho_mean[j])) * ( ((gravpos[1][i]-xprism[l]) * log(0-zprism[m]+r)) + ((0-zprism[m]) * log(gravpos[1][i]-xprism[l]+r)) - ((gravpos[2][i]-yprism[n]) * atan( ((gravpos[1][i]-xprism[l])*(0-zprism[m]))/((gravpos[2][i]-yprism[n])*r))) );
						}*/
						
						/* calculate gzz component (Nagy, 2000)*/
						/*if(GRAV_TYPE==2){
						gztmp += (double)(s*G*(rho[j][k]-rho_mean[j])) * ( -atan( ((gravpos[1][i]-xprism[l])*(0-zprism[m])) / ((gravpos[2][i]-yprism[n])*r) ) );
						}*/
