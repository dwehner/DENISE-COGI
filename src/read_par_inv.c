...
extern int INV_STF, NTR_STF;
extern float PRO, FC_START, FC_END, EPS_STF, OFFSETC; 
extern int TIMEWIN;
...
for (i=1;i<=nstage;i++){
      fscanf(fp,"%f%i%f%f%i%i%f%f%f%i%i%i%i%f%i%i%i%i%f%i%i%f%f%f",&PRO,&TIME_FILT,&FC_START,&FC_END,&ORDER,&TIMEWIN,&GAMMA,&TWLENGTH_MINUS,&TWLENGTH_PLUS,&INV_VP_ITER,&INV_VS_ITER,&INV_RHO_ITER,&SPATFILTER,&WD_DAMP,&EPRECOND,&LNORM,&INV_STF,&NTR_STF,&EPS_STF,&NORMALIZE,&OFFSET_MUTE,&OFFSETC,&SCALERHO,&GAMMA_GRAV);
}
...
if(MYID==0){
   ...
   printf("   LNORM==8 corresponds to logarithmic envelope\n\n");
   printf(" Used LNORM=%d\n\n",LNORM);
