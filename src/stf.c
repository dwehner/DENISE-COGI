...
void stf(float **sectionvy_obs, float **sectionvy, int ntr_glob, int ishot, int ns, int iter, int nshots, float **signals, int **recpos, float **srcpos){ 
	...
	extern int SEIS_FORMAT, MYID, NT, NORMALIZE, TIMEWIN, NTR_STF;
	...
	/* declaration of variables for trace killing */
        float *STF_vector;
	int ** kill_tmp, *kill_vector, h, j, i, Npad, ntr_stf;
	...
	/* trace killing ends here */
        ...
        if(NTR_STF<0){ntr_stf=ntr_glob;}
        if(NTR_STF>0){ntr_stf=NTR_STF;}
        ...
        for(i=1;i<=ntr_stf;i++){
           ...
           for(j=0;j<Npad;j++){
              ...
              if(j<ns){
	      	 ...
	      	 in_model[j] = (double) (sectionvy[i][j+1]);
                 in_data[j]  = (double) (sectionvy_obs[i][j+1]);	
              }
              else{
	      	in_model[j] = 0.0;
                 in_data[j] = 0.0;
           ...
           for(j=0;j<Npad;j++){
              ...
              sumn[j] += out_model[j]*conj(out_data[j]);
              sumd[j] += out_model[j]*conj(out_model[j]);
        ...
        for(j=0;j<Npad;j++){
           ...
           D_s[j] = sumn[j]/(sumd[j]+EPS_STF*maxdr);

           /*D_s[j] = D_ss_fd[j]*(sumn[j]/(sumd[j]+EPS_STF*maxdr));*/
        ...
        h=ns-1;
        for(j=0;j<ns;j++){
           STF_vector[j+1]=creal(D_s_td[h+1]);
           h--;
        }
